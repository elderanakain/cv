\documentclass[12pt, a4paper]{article}

\input{design}
\input{const}

% Language
\usepackage[english]{babel}
\usepackage[utf8]{inputenc}
\usepackage{newunicodechar}

% Font
\usepackage[sfdefault]{roboto}

% Page
\usepackage[a4paper, total={6in, 8in}, margin=1cm, showframe]{geometry}
\pagenumbering{gobble}

% Colour
\usepackage{xcolor}
\definecolor{ultramarine}{rgb}{0.07, 0.04, 0.56}

% Image
\usepackage{graphicx}
\graphicspath{ {images/} }
\usepackage{svg}
\usepackage{float}

% Table
\usepackage{array}

\usepackage{hyperref}
\hypersetup{
    colorlinks=true,
    urlcolor=ultramarine
}

\urlstyle{same}

% Metadata
\title{CV}
\author{Arcadii Rubailo}

% Global settings
\setlength{\intextsep}{0pt}
\setlength{\parindent}{0pt}

\setlength{\tabcolsep}{2pt}

% Content
\begin{document}
\begin{minipage}[t]{0.35\textwidth}
    \begin{figure}[H]
        \vspace*{-12pt}
        \includegraphics[width=\textwidth]{profile}
    \end{figure}
    
    \begin{center}
        \headlineSix{Arcadii Rubailo}
    \end{center}
    
    \begin{tabular}{ c l }
        \icon{icon_email}           &   rubailo.arcadii@gmail.com                                               \rowend
        \icon{icon_phone}           &   +420 775 087 503                                                        \rowend
        \icon{icon_home}            &   Videnska 22d, Brno                                                      \\
                                    &   Czech Republic                                                          \rowend
        \icon{icon_flag}            &   Moldova                                                                 \rowend
        \icon[15pt]{icon_linkedin}  &   \href{https://www.linkedin.com/in/arcadii-rubailo}{/arcadii-rubailo}    \rowend
        \icon[18pt]{icon_github}    &   \href{https://github.com/elderanakain}{/elderanakain}                   \rowend
        \icon{icon_list}            &   Programming languages:                                                  \\
                                    &   Kotlin, Java.                                                           \rowend
                                    &   Development tools:                                                      \\
                                    &   Android Studio,                                                         \\
                                    &   Zeplin, Jira, Figma.                                                    \rowend
        \icon{icon_language}        &   Russian - Native                                                        \\  
                                    &   English - C1                                                            \\  
                                    &   Romanian - B1                                                           \\  
                                    &   Czech - A1                                                              \rowend
    \end{tabular}
\end{minipage}
\hspace{15pt}
\begin{minipage}[t]{0.6\textwidth}
    \sectionTitle{Experience}{icon_work}
    
    \begin{subsec}{\kiwiUrl{Kiwi.com}}{07.2018 – Present}
        \> \kiwiAppUrl{Kiwi.com Android app}, booking main developer. \\
        \> Tech stack: Kotlin/Java, MVVM, Realm, ARCore, \\
        \> Dagger2/Koin, Databinding\&LiveData, Moshi/Gson, \\
        \> RxJava2/Flow, Jetpack Libraries, Retrofit2+OkHttp3. \\
    \end{subsec}
    
    \begin{subsec}{\crunchyrollUrl{Crunchyroll}}{01.2017 – 08.2017}
        \> \vrvAppUrl{VRV} and \crunchyrollAppUrl{Crunchyroll} Android apps development. \\
        \> Tech stack: Java/Kotlin, Retrofit2+OkHttp3, Chromecast, \\
        \> MVP, Fabric, Exoplayer, Firebase, Branch.io, \\
        \> Outbound.io, JUnit4, Mockito1. \\
    \end{subsec}
    
    \begin{subsec}{\yopesoUrl{Yopeso}}{08.2016 – 01.2017}
        \> \limangoAppUrl{limango} Android app development. \\
        \> Tech stack: Java, Retrofit+OkHttp3, Firebase, RxJava, \\
        \> MVP, JUnit4+Mockito2, Dagger2, SQLite. \\
    \end{subsec}
    
    \begin{subsec}{\travodUrl{Travod}}{10.2015 – 08.2016}
        \> Technical support, DTP and project management. \\
        \> Tech stack: Photoshop, InDesign, SDL Trados. \\
    \end{subsec}
     
    \begin{subsec}{\aursoftUrl{Aursoft}}{02.2015 – 12.2015}
        \> MyBebe Android app development. \\
        \> Tech stack: Java, Support Library, Volley, Picasso, SQLite. \\
    \end{subsec}
    
    \sectionTitle{Education}{icon_school}
    
    \begin{subsec}{\muniUrl{Masaryk University}}{2017 – 2019}
        \> MSc, \muniFiUrl{Faculty of Informatics} \\
        \> \muniSsmeFiUrl{Service Science, Management and Engineering} \\
    \end{subsec}
    
    \begin{subsec}{\usmUrl{State University of Moldova}}{2014 – 2017}
        \> BSc, \fmiUsmUrl{Mathematics and Computer Science} \\
        \> \infFmiUsmUrl{Computer science} \\
    \end{subsec}
\end{minipage}
\end{document}
